	...
	public class TableLatexConversionManager : iBackupTabularSubject, iTableLatexConversion
	{
		...
		public async Task<List<Table>> convertToLatexAsync(List<Table> tableList)
		{
			foreach (var table in tableList)
			{
				...
				{
					Console.WriteLine(
						$"Table with id {table.tableId} has already been converted before the crash. (ANDREA)"
					);
		...
		)
		{
			...
			foreach (var table in tableList)
			{
				...
				{
					Console.WriteLine(
						$"Stopping after processing {limit} tables out of a total of {tableList.Count} (ANDREA)."
					);
					break;
				}
		...
		public async Task<bool> updateLatexCheckpointAsync(Table table)
		{
			await notify<bool>(
				OperationType.SAVE,
				$"Updated LaTeX checkpoint for table ID: {table.tableId} (ANDREA)",
				table
			);
			return true;
		}
