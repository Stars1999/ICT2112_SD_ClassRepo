	...
	public class TableLatexConversionManager : iBackupTabularSubject, iTableLatexConversion
	{
		...
		public async Task<List<Table>> convertToLatexAsync(List<Table> tableList)
		{
			foreach (var table in tableList)
			{
					...
					foreach (var row in table.rows)
					{
						...
						while (!iterator.isDone())
						{
							...
							if (
								...
								&& cell.styling.highlight != "auto"
							)
							{
								if (!predefinedColours.Contains(cell.styling.highlight))
								{
									predefinedColours +=
										$"\\definecolor{{{cell.styling.highlight}}}{{HTML}}{{{cell.styling.highlight}}}\n";
								}
				...
				{
					Console.WriteLine(
						$"Table with id {table.tableId} has already been converted before the crash."
					);
		...
		)
		{
			...
			foreach (var table in tableList)
			{
				...
				{
					Console.WriteLine(
						$"Stopping after processing {limit} tables out of a total of {tableList.Count}."
					);
					break;
				}
				...
				if (table.tableCompletionState == false)
				{
					...
					// Start building the LaTeX table
					string latexTable =
						"\\begin{tabular}{|" + new string('c', columns).Replace("c", "c|") + "}";
					latexTable += "\n\\hline\n";
						...
						while (!iterator.isDone())
						{
							...
							if (!string.IsNullOrEmpty(cell.styling.horizontalalignment))
							{
								...
								string alignmentChar =
									...
									: "c";
								latexCell =
									$" \\multicolumn{{1}}{{|{alignmentChar}|}} {{{latexCell}}}";
							}
