		...
		)
		{
			...
			foreach (var processedTable in processedTables)
			{
				...
				if (tableNode != null)
				{
					...
					Console.WriteLine(
						$"Updated group 3 table node {tableNode.GetNodeId()} with LaTeX content of {processedTable.latexOutput}."
					);
				...
				{
					Console.WriteLine(
						$"Warning: No matching table node found for table ID {processedTable.tableId}."
					);
				}
			}
		...
		public async Task<bool> deleteTables(List<Table> tables)
		{
			foreach (var table in tables)
			{
				await notify<bool>(
					...
					"Deleting tables since no longer needed.",
					table
				);
			}
			return true;
