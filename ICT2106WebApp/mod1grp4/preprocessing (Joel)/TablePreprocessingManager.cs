namespace ICT2106WebApp.mod1grp4 {
    public class TablePreprocessingManager : iBackupTabularSubject, iPreprocessedTable
    {
        ...
        public async Task<List<Table>> fixTableIntegrity(List<Table> tables)
        {
            ...
            {
                await notify<bool>(OperationType.SAVE, $"Table integrity has been fixed by TablePreprocessingManager and table with id {table.tableId} saved as backup (JOEL)", table);
            }
        ...
        public async Task<List<Table>> recoverBackupTablesIfExist(List<Table> tablesFromNode)
        {
            ...
            var backupTables = await notify<List<Table>>(OperationType.RETRIEVE, "Checking if backup tables exist: whether ids given to me tallies with what i have in database (JOEL)", tablesFromNode);
            ...
            {
                Console.WriteLine("Backup Tables do not exist. No crash has occurred previously. Proceeding with tables from node. (JOEL)");
                return tablesFromNode;
            }

            Console.WriteLine("Backup Tables exists. Crash has occurred previously. Retrieving backup tables instead. (JOEL)");
            return backupTables;
        }
    }
