	...
	public class TablePreprocessingManager : iBackupTabularSubject, iPreprocessedTable
	{
		...
		public async Task<List<Table>> fixTableIntegrity(List<Table> tables)
		{
			...
			foreach (var table in tables)
			{
				await notify<bool>(
					OperationType.SAVE,
					$"Table integrity has been fixed by TablePreprocessingManager and table with id {table.tableId} saved as backup",
					table
				);
			}
		...
		public async Task<List<Table>> recoverBackupTablesIfExist(List<Table> tablesFromNode)
		{
			...
			var backupTables = await notify<List<Table>>(
				...
				"Checking if backup tables exist: whether ids given to me tallies with what i have in database",
				tablesFromNode
			);
			...
			{
				Console.WriteLine(
					"Backup Tables do not exist. No crash has occurred previously. Proceeding with tables from node."
				);
				return tablesFromNode;
			}

			Console.WriteLine(
				"Backup Tables exists. Crash has occurred previously. Retrieving backup tables instead."
			);
			return backupTables;
		}
	}
