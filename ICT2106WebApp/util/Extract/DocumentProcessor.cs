...
public class DocumentProcessor : IDocumentUpdateNotify
{
	...
	private readonly IFooterExtractor _footerExtractor;
	private readonly ILayoutExtractor _layoutExtractor;
	...
	private const string OutputJsonPath = "output.json";

	public string jsonString { get; private set; }
	public JArray documentArray { get; private set; }

	public DocumentProcessor(
		IHeaderExtractor headerExtractor,
		IFooterExtractor footerExtractor,
		ILayoutExtractor layoutExtractor,
	...
	)
	{
		...
		_footerExtractor = footerExtractor;
	...
	public async Task notifyUpdatedDocument(Docx docx)
	{
		Console.WriteLine($"📢 Notify Document updated: {docx.GetDocxAttributeValue("title")}");
	...
	public async Task saveDocumentToDatabase(string filePath)
	{
		...
		if (!filePath.EndsWith(".docx"))
			throw new ArgumentException("Only .docx files allowed");

		byte[] fileData = await File.ReadAllBytesAsync(filePath);

		var docx = new Docx().CreateDocx(
			Path.GetFileNameWithoutExtension(filePath),
			Path.GetFileName(filePath),
			fileData
		);

		await _documentRepository.SaveDocument(docx);
	}

	public async Task saveJsonToDatabase(string jsonPath)
	{
		await _documentRepository.SaveJson(jsonPath);
	}

	public async Task<List<object>> ParseDocument(string filePath)
	{
		...
		foreach (var element in doc.MainDocumentPart.Document.Body.Elements())
		{
			...
			{
				elements.Add(_paragraphExtractor.Extract(paragraph, doc, ref haveBibliography));
			}
			else if (element is DocumentFormat.OpenXml.Wordprocessing.Table table)
			{
			...
			{
				elements.AddRange(_imageExtractor.ExtractImages(doc, drawing));
			}
		}
		...
		var layout = new Dictionary<string, object>
		{
			...
			{
				"styling",
				new List<object> { _layoutExtractor.ExtractLayout(doc) }
			},
		};

		elements.Insert(0, layout);
		elements.Insert(0, root);

		var documentData = new
		{
			metadata = _metadataExtractor.ExtractMetadata(doc, fullPath),
			headers = _headerExtractor.ExtractHeaders(doc),
			footers = _footerExtractor.ExtractFooters(doc),
			document = elements,
		};

		jsonString = _jsonSerializer.SerializeToFile(documentData, OutputJsonPath);
		JObject jsonObj = JObject.Parse(jsonString);
		documentArray = (JArray)jsonObj["document"];
